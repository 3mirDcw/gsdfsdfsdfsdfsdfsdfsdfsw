...
let s = message.guild.id
    let lastDaily = await db.fetch(`lastDaily_${message.author.id+s}`);
    if (lastDaily !== null && cooldown - (Date.now() - lastDaily) > 0) {
      ...
      let kS = bot.emojis.get(bot.emojiler.kapalıS);
        ...
        const Durum = await db.fetch(`bonus_${message.author.id+s}`);
      ...
      const tplnB = await db.fetch(`bonus_${message.author.id+s}`); 
        var tbns = ''
    ...
    //   if (tplnB == '1' || tplnB == '2' || tplnB == '3' || tplnB == '4' || tplnB === null || tplnB == '0') { var tbns = `0` }
        ...
        db.set(`lastDaily_${message.author.id+s}`, Date.now());
      
        db.add(`bonus_${message.author.id+s}`, 1)
        db.add(`paracık_${message.author.id+s}`, amount);
        db.add(`paracık_${message.author.id+s}`, bns == 5 ? "500" : "0");
        db.add(`paracık_${message.author.id+s}`, meslek === null  ? "0" : meslek);
