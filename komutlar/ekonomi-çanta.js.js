...
let s = message.guild.id
  ...
  let boksE = await db.fetch(`boksE_${user.id+s}`);
  let kalem = await db.fetch(`kalem_${user.id+s}`);
  let kapışC = await db.fetch(`kapışC_${user.id+s}`);
  let rozet = await db.fetch(`memberBadge6_${user.id+s}`);
...
exports.help = {
  name: 'çanta',
  description: 'Aldığınız eşyaları g��sterir.',
  usage: 'çanta'
};